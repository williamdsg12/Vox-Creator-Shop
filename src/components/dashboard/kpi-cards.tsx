...
export function KpiGrid() {
  const { data: apiKpis, isLoading } = useQuery({
    queryKey: ["kpis-metrics"],
    queryFn: async () => {
      try {
        const res = await fetchApi<Kpi[]>("/analytics/kpis");
        return Array.isArray(res) && res.length ? res : defaultKpis;
      } catch (err) {
        return defaultKpis;
      }
    },
  ...
  const kpis = apiKpis || defaultKpis;
    ...
    <section className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {isLoading
          ...
          : kpis.map((k) => {
              const IconComp = k.icon || iconMap[k.label] || TrendingUp;
              const accentKey = k.accent || "neon";
              const strokeColor = strokeMap[accentKey] || strokeMap.neon;
              const sparkData = k.spark && k.spark.length ? k.spark : [1, 2, 3, 4, 5];
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className={cn("grid h-9 w-9 place-items-center rounded-lg", accent[accentKey])}>
